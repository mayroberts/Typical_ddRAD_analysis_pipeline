...
When analyzing ddRAD data, use PCA if you want to identify major patterns of variation across many correlated genetic markers, while PCoA is preferable when you want to visualize the distances between samples based on a pre-calculated genetic distance matrix, particularly when the relationships between samples are not necessarily linear or when you have a large number of rare alleles. 
